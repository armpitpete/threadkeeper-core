...
39. **CW-39 — Live repository identity pinning.** `New` must open and retain a live handle to the canonical Git-directory root for the lifetime of the `Reader`, so deleting the pathname cannot free that filesystem object for inode/file-ID reuse while the Reader remains capable of authority work. Every later Core Git invocation must hold that live handle and require the current pathname to identify the same still-open filesystem object. A deleted-and-recreated ordinary repository at the same pathname must fail before Git is invoked, even if the filesystem would otherwise recycle identifiers. Closing the Reader must release the handle and permanently make that Reader fail closed.
