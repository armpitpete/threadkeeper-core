...
- fresh bounded authoritative reconciliation for stale Prepare results and pre-CAS Accept candidate/quarantine failures that may race a concurrent identical acceptance, preserving exactly-one `accepted` / recovered `already_accepted` semantics, preventing stale prepared candidates from surviving a known concurrent acceptance, and cleaning staging/bound material abandoned by that reconciliation;
...
- quarantine `Ensure` tolerates only a transient short exact-prefix file from an in-progress identical creator and otherwise fails closed, preventing identical concurrent prepares from misclassifying a partially written shared bound capability as `QUARANTINE_CONFLICT`;
...
- the current repair fresh-reconciles authoritative state before returning stale Prepare results or pre-CAS Accept candidate/quarantine failures, reconstructs exact concurrent acceptance as `already_accepted`, removes prepared quarantine material that became obsolete through a known concurrent acceptance, cleans staging material when authority moves before candidate construction, gives identical concurrent Prepare operations separate temporary staging entries, and makes identical shared bound-capability creation safe against partial-write observation. Because this again changes the reviewed authority boundary, its final exact merged commit requires a completely fresh independent hostile review; CI/self-review cannot convert the prior FAIL into PASS;
