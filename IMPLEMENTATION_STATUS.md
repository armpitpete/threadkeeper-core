...
- fresh bounded authoritative reconciliation for stale Prepare results and pre-CAS Accept candidate/quarantine failures that may race a concurrent identical acceptance, preserving exactly-one `accepted` / recovered `already_accepted` semantics and preventing stale prepared candidates from surviving a known concurrent acceptance;
