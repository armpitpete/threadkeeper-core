...
func recoveryRestoreVerifyCommand(args[]string)error{if len(args)<3||len(args)>4{return fmt.Errorf("expected original recovery-proof file, restored ledger git directory, provenance file and optional authoritative ref")};proofRaw,err:=os.ReadFile(args[0]);if err!=nil{return err};original,err:=restoreproof.DecodeRecoveryProof(proofRaw);if err!=nil{return err};provenanceRaw,err:=os.ReadFile(args[2]);if err!=nil{return err};provenance,err:=restoreproof.DecodeProvenance(provenanceRaw);if err!=nil{return err};readerArgs:=[]string{args[1]};if len(args)==4{readerArgs=append(readerArgs,args[3])};r,err:=ledgerReaderFromArgs(readerArgs);if err!=nil{return err};defer r.Close();report,verifyErr:=restoreproof.Verify(context.Background(),r,original,provenance);if report!=nil{if err:=writeIndentedJSON(report);err!=nil{return err}};return verifyErr}
