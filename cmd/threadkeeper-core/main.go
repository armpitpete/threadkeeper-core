...
func main() {
	...
	var err error
	switch os.Args[1] {
	case "version": err=printVersion()
	case "check-json": err=oneFile(os.Args[2:],func(b []byte)error{return strictjson.Validate(b)})
	case "canonicalize": err=oneFileOut(os.Args[2:],canonicaljson.Canonicalize)
	case "digest": err=digestCommand(os.Args[2:])
	case "validate": err=validateCommand(os.Args[2:])
	case "genesis-check": err=genesisCheckCommand(os.Args[2:])
	case "evidence-check": err=evidenceCheckCommand(os.Args[2:])
	case "review-bundle": err=reviewBundleCommand(os.Args[2:])
	case "health-check": err=healthCheckCommand(os.Args[2:])
	case "ledger-inspect": err=ledgerInspectCommand(os.Args[2:])
	case "ledger-recovery-proof": err=ledgerRecoveryProofCommand(os.Args[2:])
	case "recovery-compare": err=recoveryCompareCommand(os.Args[2:])
	case "authority-write": err=service.RequireAuthorityWritesEnabled()
	default: usage(); os.Exit(2)
	}
	if err!=nil{fmt.Fprintln(os.Stderr,err);os.Exit(1)}
}

func usage(){fmt.Fprintln(os.Stderr,"usage: threadkeeper-core <version|check-json|canonicalize|digest|validate|genesis-check|evidence-check|review-bundle|health-check|ledger-inspect|ledger-recovery-proof|recovery-compare|authority-write> ...")}
...
func ledgerInspectCommand(args []string)error{r,err:=ledgerReaderFromArgs(args);if err!=nil{return err};manifest,err:=ledger.Replay(context.Background(),r);if err!=nil{return err};return writeIndentedJSON(manifest)}
func ledgerRecoveryProofCommand(args []string)error{r,err:=ledgerReaderFromArgs(args);if err!=nil{return err};proof,err:=ledger.ProveRecovery(context.Background(),r);if err!=nil{return err};return writeIndentedJSON(proof)}
