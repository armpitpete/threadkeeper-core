...
func TestImplementationStatusMatchesExecutableSafetyBoundary(t *testing.T){
	...
	text:=string(raw)
	for _,required:=range []string{"Git ledger replay","candidate-write","AUTHORITY_WRITES_DISABLED","genesis","coverage","fork recovery","destructive non-empty bare-ledger","source adapter","provenance graph","portable Core export","reference CLI","overload signalling"}{if !strings.Contains(text,required){t.Fatalf("implementation status missing %q",required)}}
