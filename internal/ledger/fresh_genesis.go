...
func validateFreshGenesisSeedPath(p string) error {
	...
	if !allowedSchema && !allowedBinding {
		return fmt.Errorf("FRESH_GENESIS_INVALID: seed path %q is outside initial schema/reducer-binding namespaces", p)
	}
	return nil
}
