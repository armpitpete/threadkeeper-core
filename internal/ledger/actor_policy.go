...
func validateInitialActorPolicyHistory(ctx context.Context, r *gitledger.Reader, history []gitledger.Commit, root genesis.Root) error {
	if len(history) == 0 {
		return fmt.Errorf("AUTH_POLICY_INVALID: authoritative history is empty")
	}
	rootCommit := history[0].ID
	changes, err := r.ImmutableJSONAdditions(ctx, rootCommit, actorauth.LedgerPolicyPrefix)
	if err != nil {
		return fmt.Errorf("AUTH_POLICY_INVALID: root actor-policy tree: %w", err)
	}
	if len(changes) != 1 || changes[0] != actorauth.LedgerPolicyPath {
		return fmt.Errorf("AUTH_POLICY_INVALID: Genesis commit %s must add exactly %q, got %v", rootCommit, actorauth.LedgerPolicyPath, changes)
	}
	raw, err := r.ReadRegularFile(ctx, rootCommit, actorauth.LedgerPolicyPath)
	if err != nil {
		return fmt.Errorf("AUTH_POLICY_INVALID: read root actor policy: %w", err)
	}
	doc, _, err := actorauth.ParsePolicyDocument(raw, root.LedgerID)
	if err != nil {
		return err
	}
	...
	for _, commit := range history[1:] {
		...
		if err != nil {
			return fmt.Errorf("AUTH_POLICY_IMMUTABLE: commit %s: %w", commit.ID, err)
		}
		if len(later) != 0 {
			return fmt.Errorf("AUTH_POLICY_IMMUTABLE: commit %s changes root actor-policy material %v", commit.ID, later)
		}
	}
	return nil
}
...
func LoadCurrentActorPolicy(ctx context.Context, r *gitledger.Reader) (*ActorPolicySnapshot, error) {
	...
	doc, policy, err := actorauth.ParsePolicyDocument(raw, manifest.GenesisRoot.LedgerID)
	if err != nil {
		return nil, err
	}
	sourceEvent := ""
	if state, exists := manifest.GovernedRecords[actorauth.PolicyTarget]; exists {
		...
		doc, policy, err = actorauth.ParsePolicyDocument(state.Value, manifest.GenesisRoot.LedgerID)
		if err != nil {
			return nil, err
		}
